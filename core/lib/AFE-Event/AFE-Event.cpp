 ...
 */
  ...
  if (_Firmware->timer->minutes > AFE_TIMER_ONE_HOUR) {
    ...
    if (_Firmware->Device->getMode() == AFE_MODE_NORMAL) {
#ifdef DEBUG
      _Firmware->Debugger->printInformation(F("Checking access to WAN"),
                                            F("EVENT"));
#endif
...
void AFEEvent::connectedToNetwork(void) {
  ...
  if (_Firmware->Device->getMode() == AFE_MODE_NORMAL) {
    if (_Firmware->Device->configuration.api.domoticz) {
      ...
      _Firmware->Debugger->printInformation(
          F("Domoticz HTTP API boot actions triggering"), F("EVENT"));
#endif
...
#ifdef DEBUG
      _Firmware->Debugger->printInformation(
          F("Sending current gate state to Domoticz"), F("EVENT"));
#endif
...
#ifdef DEBUG
      _Firmware->Debugger->printInformation(
          F("Sending current state of contactrons to Domoticz"), F("EVENT"));
#endif
...
#ifdef DEBUG
      _Firmware->Debugger->printInformation(
          F("Sending current state of switches to Domoticz"), F("EVENT"));
#endif
...
#ifdef DEBUG
      _Firmware->Debugger->printInformation(
          F("Sending current state of binary sensors to Domoticz"), F("EVENT"));
#endif
      for (uint8_t i = 0;
...
#ifdef DEBUG
      _Firmware->Debugger->printInformation(
          F("Sending current state of regulator to Domoticz"), F("EVENT"));
#endif
...
#ifdef DEBUG
      _Firmware->Debugger->printInformation(
          F("Sending current state of relays to Domoticz"), F("EVENT"));
#endif
...
#ifdef DEBUG
    _Firmware->Debugger->printInformation(
        F("Post WiFi Connection actions completed"), F("EVENT"));
#endif
  }
#endif
...
void AFEEvent::disconnectedFromNetwork(void) {
#ifdef DEBUG
  _Firmware->Debugger->printInformation(F("Network disconnected: triggered"),
                                        F("EVENT"));
#endif
#ifdef AFE_CONFIG_HARDWARE_LED
  _Hardware->SystemLed->on();
  ...
  /* Forcing disconnection from MQTT Broker */
  if (_Firmware->Device->getMode() == AFE_MODE_NORMAL &&
      _Firmware->Device->configuration.api.mqtt) {
...
#ifdef DEBUG
      _Firmware->Debugger->printInformation(F("MQTT Connected: triggered"),
                                            F("EVENT"));
#endif
...
#ifdef DEBUG
      _Firmware->Debugger->printInformation(F("MQTT Disconnected: triggered"),
                                            F("EVENT"));
