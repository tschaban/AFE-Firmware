...
public:
    ...
    AFESoftwareSerial(int8_t rxPin, int8_t txPin = -1, bool invert = false);
    AFESoftwareSerial(const AFESoftwareSerial&) = delete;
    AFESoftwareSerial& operator= (const AFESoftwareSerial&) = delete;
    virtual ~AFESoftwareSerial();
...
private:
    ...
    static void rxBitISR(AFESoftwareSerial* self);
    static void rxBitSyncISR(AFESoftwareSerial* self);
    ...
    std::unique_ptr<circular_queue<uint32_t, AFESoftwareSerial*> > m_isrBuffer;
    const Delegate<void(uint32_t&&), AFESoftwareSerial*> m_isrBufferForEachDel = { [](AFESoftwareSerial* self, uint32_t&& isrCycle) { self->rxBits(isrCycle); }, this };
