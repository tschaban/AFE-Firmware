...
Version 6.16.1 from https://www.arduino.cc/reference/en/libraries/espsoftwareserial/
...
#ifndef ESP32
uint32_t AFESoftwareSerial::m_savedPS = 0;
#else
portMUX_TYPE AFESoftwareSerial::m_interruptsMux = portMUX_INITIALIZER_UNLOCKED;
#endif

inline void IRAM_ATTR AFESoftwareSerial::disableInterrupts()
{
#ifndef ESP32
...
inline void IRAM_ATTR AFESoftwareSerial::restoreInterrupts()
{
#ifndef ESP32
    xt_wsr_ps(m_savedPS);
#else
...
AFESoftwareSerial::SoftwareSerial() {
    m_isrOverflow = false;
    m_rxGPIOPullupEnabled = true;
}

AFESoftwareSerial::SoftwareSerial(int8_t rxPin, int8_t txPin, bool invert)
{
    m_isrOverflow = false;
    m_rxGPIOPullupEnabled = true;
    m_rxPin = rxPin;
...
bool AFESoftwareSerial::isValidGPIOpin(int8_t pin) {
#if defined(ESP8266)
...
bool AFESoftwareSerial::isValidRxGPIOpin(int8_t pin) {
...
bool AFESoftwareSerial::isValidTxGPIOpin(int8_t pin) {
    return isValidGPIOpin(pin)
#if defined(ESP32)
#ifdef CONFIG_IDF_TARGET_ESP32
        && (pin < 34)
...
bool AFESoftwareSerial::hasRxGPIOPullUp(int8_t pin) {
#if defined(ESP32)
...
void AFESoftwareSerial::setRxGPIOPullUp() {
    if (m_rxValid) {
        pinMode(m_rxPin, hasRxGPIOPullUp(m_rxPin) && m_rxGPIOPullupEnabled ? INPUT_PULLUP : INPUT);
    }
}

void AFESoftwareSerial::begin(uint32_t baud, SoftwareSerialConfig config,
    ...
    if (isValidRxGPIOpin(m_rxPin)) {
        ...
        m_isrBuffer.reset(new circular_queue<uint32_t, AFESoftwareSerial*>((isrBufCapacity > 0) ?
...
uint32_t AFESoftwareSerial::baudRate() {
    return ESP.getCpuFreqMHz() * 1000000UL / m_bitCycles;
}

void AFESoftwareSerial::setTransmitEnablePin(int8_t txEnablePin) {
...
void AFESoftwareSerial::enableIntTx(bool on) {
    m_intTxEnabled = on;
}

void AFESoftwareSerial::enableRxGPIOPullup(bool on) {
    m_rxGPIOPullupEnabled = on;
    setRxGPIOPullUp();
}

void AFESoftwareSerial::enableTx(bool on) {
...
int AFESoftwareSerial::read(uint8_t* buffer, size_t size) {
...
size_t AFESoftwareSerial::readBytes(uint8_t* buffer, size_t size) {
...
void AFESoftwareSerial::lazyDelay() {
...
void IRAM_ATTR AFESoftwareSerial::preciseDelay() {
    while ((ESP.getCycleCount() - m_periodStart) < m_periodDuration) {}
    m_periodDuration = 0;
    m_periodStart = ESP.getCycleCount();
}

void IRAM_ATTR AFESoftwareSerial::writePeriod(
...
size_t AFESoftwareSerial::write(uint8_t byte) {
    return write(&byte, 1);
}

size_t AFESoftwareSerial::write(uint8_t byte, SoftwareSerialParity parity) {
    return write(&byte, 1, parity);
}

size_t AFESoftwareSerial::write(const uint8_t* buffer, size_t size) {
    return write(buffer, size, m_parityMode);
}

size_t IRAM_ATTR AFESoftwareSerial::write(const uint8_t* buffer, size_t size, SoftwareSerialParity parity) {
    if (m_rxValid) { rxBits(); }
...
bool AFESoftwareSerial::overflow() {
    bool res = m_overflow;
    m_overflow = false;
    return res;
}
...
void AFESoftwareSerial::rxBits(const uint32_t isrCycle) {
...
void IRAM_ATTR AFESoftwareSerial::rxBitISR(SoftwareSerial* self) {
...
void IRAM_ATTR AFESoftwareSerial::rxBitSyncISR(SoftwareSerial* self) {
...
void AFESoftwareSerial::onReceive(Delegate<void(int available), void*> handler) {
    receiveHandler = handler;
}

void AFESoftwareSerial::perform_work() {
    if (!m_rxValid) { return; }
    rxBits();
    if (receiveHandler) {
        int avail = m_buffer->available();
