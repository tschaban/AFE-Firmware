...
#endif
#ifdef DEBUG
    Debugger->printWarning(F("Forcing disconnection from MQTT Broker on object level"), F("MQTT"));
#endif
...
{
#ifdef DEBUG
  Debugger->printInformation(F("Sending message: device is connected"),F("MQTT"));
#endif
...
#ifdef DEBUG
    Serial << endl
           << F("WARN: MQTT: Disconnected from MQTT Broker : ");
