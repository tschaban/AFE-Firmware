...
boolean AFEAPIHTTPDomoticz::publishRelayState(uint8_t id) {
  return _initialized
             ? sendSwitchCommand(_Hardware->Relay[id]->configuration->domoticz.idx,
...
boolean AFEAPIHTTPDomoticz::publishSwitchState(uint8_t id) {
  return _initialized
             ? sendSwitchCommand(_Hardware->Switch[id]->configuration->domoticz.idx,
...
void AFEAPIHTTPDomoticz::publishADCValues(uint8_t id) {
  if (_initialized) {
    ...
    if (_Hardware->AnalogInput[id]->configuration->domoticz.percent > 0) {
      ...
      sendCustomSensorCommand(_Hardware->AnalogInput[id]->configuration->domoticz.percent,
                              value);
    }
    if (_Hardware->AnalogInput[id]->configuration->domoticz.voltage > 0) {
      sprintf(value, "%-.4f", _Hardware->AnalogInput[id]->data->voltage);
      sendCustomSensorCommand(_Hardware->AnalogInput[id]->configuration->domoticz.voltage,
                              value);
    }
    if (_Hardware->AnalogInput[id]->configuration->domoticz.voltageCalculated > 0) {
      sprintf(value, "%-.4f", _Hardware->AnalogInput[id]->data->voltageCalculated);
      sendCustomSensorCommand(
          _Hardware->AnalogInput[id]->configuration->domoticz.voltageCalculated, value);
    }
    if (_Hardware->AnalogInput[id]->configuration->domoticz.raw > 0) {
      sprintf(value, "%-d", _Hardware->AnalogInput[id]->data->raw);
      sendCustomSensorCommand(_Hardware->AnalogInput[id]->configuration->domoticz.raw,
                              value);
    }
  }
}
...
void AFEAPIHTTPDomoticz::publishADCValues() {
  if (_initialized) {
    ...
    if (_Hardware->AnalogInput->configuration->domoticz.percent > 0) {
      ...
      sendCustomSensorCommand(_Hardware->AnalogInput->configuration->domoticz.percent,
                              value);
    }
    if (_Hardware->AnalogInput->configuration->domoticz.voltage > 0) {
      sprintf(value, "%-.4f", _Hardware->AnalogInput->data->voltage);
      sendCustomSensorCommand(_Hardware->AnalogInput->configuration->domoticz.voltage,
                              value);
    }
    if (_Hardware->AnalogInput->configuration->domoticz.voltageCalculated > 0) {
      ...
      sendCustomSensorCommand(
          _Hardware->AnalogInput->configuration->domoticz.voltageCalculated, value);
    }
    if (_Hardware->AnalogInput->configuration->domoticz.raw > 0) {
      sprintf(value, "%-d", _Hardware->AnalogInput->data->raw);
      sendCustomSensorCommand(_Hardware->AnalogInput->configuration->domoticz.raw, value);
    }
  }
}
#endif // ESP32/ESP8266
  ...
  if (_initialized) {
    ...
    if (_Hardware->AnalogInput[id]->configuration->battery.domoticz.idx > 0) {
      ...
      _ret = sendCustomSensorCommand(
          _Hardware->AnalogInput[id]->configuration->battery.domoticz.idx, value);
    }
  }
  return _ret;
}
...
boolean AFEAPIHTTPDomoticz::publishBoschBMSensorData(uint8_t id) {
  ...
  if (_initialized) {
    char value[20];
    if (_Hardware->BMEX80Sensor[id]->configuration->domoticz.temperature.idx > 0) {
      sprintf(value, "%-.2f", _Hardware->BMEX80Sensor[id]->data->temperature.value);
      sendCustomSensorCommand(
          _Hardware->BMEX80Sensor[id]->configuration->domoticz.temperature.idx, value);
    }

    if (_Hardware->BMEX80Sensor[id]->configuration->domoticz.pressure.idx > 0) {
      sprintf(value, "%-.2f;0", _Hardware->BMEX80Sensor[id]->data->pressure.value);
      sendCustomSensorCommand(
          _Hardware->BMEX80Sensor[id]->configuration->domoticz.pressure.idx, value);
    }
    if (_Hardware->BMEX80Sensor[id]->configuration->domoticz.relativePressure.idx > 0) {
      sprintf(value, "%-.2f;0", _Hardware->BMEX80Sensor[id]->data->relativePressure.value);
      sendCustomSensorCommand(
          _Hardware->BMEX80Sensor[id]->configuration->domoticz.relativePressure.idx, value);
    }

    if (_Hardware->BMEX80Sensor[id]->configuration->type != AFE_BMP180_SENSOR) {

      if (_Hardware->BMEX80Sensor[id]->configuration->domoticz.temperatureHumidity.idx >
          0) {
        sprintf(value, "%-.2f;%-.2f;%-d",
                ...
                _Hardware->BMEX80Sensor[id]->convertHumidyStatusDomoticz(
                    _Hardware->BMEX80Sensor[id]->data->humidity.value));
        sendCustomSensorCommand(
            _Hardware->BMEX80Sensor[id]->configuration->domoticz.temperatureHumidity.idx,
            value);
      }
      ...
      if (_Hardware->BMEX80Sensor[id]->configuration->domoticz.humidity.idx > 0) {
        sprintf(value, "%d", _Hardware->BMEX80Sensor[id]->convertHumidyStatusDomoticz(
                                 _Hardware->BMEX80Sensor[id]->data->humidity.value));
        sendCustomSensorCommand(
            _Hardware->BMEX80Sensor[id]->configuration->domoticz.humidity.idx, value,
            (uint8_t)_Hardware->BMEX80Sensor[id]->data->humidity.value);
      }

      if (_Hardware->BMEX80Sensor[id]->configuration->domoticz.absoluteHumidity.idx > 0) {
        sprintf(value, "%d", _Hardware->BMEX80Sensor[id]->convertHumidyStatusDomoticz(
                                 _Hardware->BMEX80Sensor[id]->data->humidity.value));
        sendCustomSensorCommand(
            _Hardware->BMEX80Sensor[id]->configuration->domoticz.absoluteHumidity.idx,
            value, (uint8_t)_Hardware->BMEX80Sensor[id]->absoluteHumidity(
                       _Hardware->BMEX80Sensor[id]->data->temperature.value,
                       _Hardware->BMEX80Sensor[id]->data->humidity.value,
      ...
      if (_Hardware->BMEX80Sensor[id]->configuration->domoticz.dewPoint.idx > 0) {
        sprintf(value, "%-.2f", _Hardware->BMEX80Sensor[id]->data->dewPoint.value);
        sendCustomSensorCommand(
            _Hardware->BMEX80Sensor[id]->configuration->domoticz.dewPoint.idx, value);
      }
      if (_Hardware->BMEX80Sensor[id]->configuration->domoticz.heatIndex.idx > 0) {
        sprintf(value, "%-.2f", _Hardware->BMEX80Sensor[id]->data->heatIndex.value);
        sendCustomSensorCommand(
            _Hardware->BMEX80Sensor[id]->configuration->domoticz.heatIndex.idx, value);
      }

      /* Perception */
      if (_Hardware->BMEX80Sensor[id]->configuration->domoticz.perception.idx > 0) {
        ...
        sendCustomSensorCommand(
            _Hardware->BMEX80Sensor[id]->configuration->domoticz.heatIndex.idx, _perception,
            _Hardware->BMEX80Sensor[id]->convertPerceptionDomoticz(_perceptionId));
      }
      ...
      if (_Hardware->BMEX80Sensor[id]->configuration->domoticz.comfort.idx > 0) {
        ...
        sendCustomSensorCommand(
            _Hardware->BMEX80Sensor[id]->configuration->domoticz.heatIndex.idx, _comfort,
    ...
    if (_Hardware->BMEX80Sensor[id]->configuration->type == AFE_BME680_SENSOR) {
      if (_Hardware->BMEX80Sensor[id]->configuration->domoticz.gasResistance.idx > 0) {
        sprintf(value, "%-.2f", _Hardware->BMEX80Sensor[id]->data->gasResistance.value);
        sendCustomSensorCommand(
            _Hardware->BMEX80Sensor[id]->configuration->domoticz.gasResistance.idx, value);
      }
      if (_Hardware->BMEX80Sensor[id]->configuration->domoticz.iaq.idx > 0) {
        sprintf(value, "%-.2f", _Hardware->BMEX80Sensor[id]->data->iaq.value);
        sendCustomSensorCommand(
            _Hardware->BMEX80Sensor[id]->configuration->domoticz.iaq.idx, value);
      }
      if (_Hardware->BMEX80Sensor[id]->configuration->domoticz.staticIaq.idx > 0) {
        sprintf(value, "%-.2f", _Hardware->BMEX80Sensor[id]->data->staticIaq.value);
        sendCustomSensorCommand(
            _Hardware->BMEX80Sensor[id]->configuration->domoticz.staticIaq.idx, value);
      }
      if (_Hardware->BMEX80Sensor[id]->configuration->domoticz.co2Equivalent.idx > 0) {
        sprintf(value, "%-.2f", _Hardware->BMEX80Sensor[id]->data->co2Equivalent.value);
        sendCustomSensorCommand(
            _Hardware->BMEX80Sensor[id]->configuration->domoticz.co2Equivalent.idx, value);
      }
      if (_Hardware->BMEX80Sensor[id]->configuration->domoticz.breathVocEquivalent.idx >
          0) {
        sprintf(value, "%-.2f",
                _Hardware->BMEX80Sensor[id]->data->breathVocEquivalent.value);
        sendCustomSensorCommand(
            _Hardware->BMEX80Sensor[id]->configuration->domoticz.breathVocEquivalent.idx,
            value);
      }
    }
...
boolean AFEAPIHTTPDomoticz::publishHPMA115S0SensorData(uint8_t id) {
  ...
  if (_initialized) {
    ...
    if (_Hardware->HPMA115S0Sensor[id]->configuration->domoticz.pm10.idx > 0) {
      ...
      sendCustomSensorCommand(
          _Hardware->HPMA115S0Sensor[id]->configuration->domoticz.pm10.idx, value,
          _Hardware->HPMA115S0Sensor[id]->data->pm10);
    }
    if (_Hardware->HPMA115S0Sensor[id]->configuration->domoticz.pm25.idx > 0) {
      sprintf(value, "%-.1f", _Hardware->HPMA115S0Sensor[id]->data->pm25);
      sendCustomSensorCommand(
          _Hardware->HPMA115S0Sensor[id]->configuration->domoticz.pm25.idx, value,
          _Hardware->HPMA115S0Sensor[id]->data->pm25);
    }
    if (_Hardware->HPMA115S0Sensor[id]->configuration->domoticz.whoPM10Norm.idx > 0) {
      sprintf(value, "%-.1f", _Hardware->HPMA115S0Sensor[id]->data->whoPM10Norm);
      sendCustomSensorCommand(
          _Hardware->HPMA115S0Sensor[id]->configuration->domoticz.whoPM10Norm.idx, value,
          _Hardware->HPMA115S0Sensor[id]->data->whoPM10Norm);
    }
    if (_Hardware->HPMA115S0Sensor[id]->configuration->domoticz.whoPM25Norm.idx > 0) {
      sprintf(value, "%-.1f", _Hardware->HPMA115S0Sensor[id]->data->whoPM25Norm);
      sendCustomSensorCommand(
          _Hardware->HPMA115S0Sensor[id]->configuration->domoticz.whoPM25Norm.idx, value,
...
boolean AFEAPIHTTPDomoticz::publishBH1750SensorData(uint8_t id) {
  ...
  if (_initialized) {
    if (_Hardware->BH1750Sensor[id]->configuration->domoticz.idx > 0) {
      ...
      sendCustomSensorCommand(_Hardware->BH1750Sensor[id]->configuration->domoticz.idx,
                              value);
      _ret = true;
    }
  }
...
boolean AFEAPIHTTPDomoticz::publishTSL2561SensorData(uint8_t id) {
  ...
  if (_initialized) {
    ...
    if (_Hardware->TSL2561Sensor[id]->configuration->domoticz.illuminance.idx > 0) {
      sprintf(value, "%d", _Hardware->TSL2561Sensor[id]->illuminance);
      sendCustomSensorCommand(
          _Hardware->TSL2561Sensor[id]->configuration->domoticz.illuminance.idx, value);
      _ret = true;
    }
    if (_Hardware->TSL2561Sensor[id]->configuration->domoticz.ir.idx > 0) {
      sprintf(value, "%d", _Hardware->TSL2561Sensor[id]->ir);
      sendCustomSensorCommand(_Hardware->TSL2561Sensor[id]->configuration->domoticz.ir.idx,
                              value);
      _ret = true;
    }
    if (_Hardware->TSL2561Sensor[id]->configuration->domoticz.broadband.idx > 0) {
      sprintf(value, "%d", _Hardware->TSL2561Sensor[id]->broadband);
      sendCustomSensorCommand(
          _Hardware->TSL2561Sensor[id]->configuration->domoticz.broadband.idx, value);
      _ret = true;
    }
  }
...
boolean AFEAPIHTTPDomoticz::publishAS3935SensorData(uint8_t id) {
  boolean _ret = false;
  if (_initialized && _Hardware->AS3935Sensorr[id]->configuration.domoticz.idx > 0) {
    char value[20]; // @TODO T6 CHeck the max size
    sprintf(value, "%-d", _Hardware->AS3935Sensorr[id]->distance);
    sendCustomSensorCommand(_Hardware->AS3935Sensorr[id]->configuration.domoticz.idx,
                            value);
    _ret = true;
  }
  return _ret;
...
void AFEAPIHTTPDomoticz::publishAnemometerSensorData() {
  if (_initialized) {
    ...
    if (_Hardware->AnemometerSensor->configuration->domoticz.idx > 0) {
      sprintf(value, "0;N;%-.2f;0;?;?", 10 * _Hardware->AnemometerSensor->lastSpeedMS);
      sendCustomSensorCommand(_Hardware->AnemometerSensor->configuration->domoticz.idx,
                              value);
    }
  }
}
...
void AFEAPIHTTPDomoticz::publishRainSensorData() {
  if (_initialized) {
    ...
    if (_Hardware->RainmeterSensor->configuration->domoticz.idx > 0) {
      sprintf(value, "%-.2f;%-.2f", _Hardware->RainmeterSensor->rainLevelLastHour * 100,
              _Hardware->RainmeterSensor->current->counter);
      sendCustomSensorCommand(_Hardware->RainmeterSensor->configuration->domoticz.idx,
                              value);
    }
  }
}
...
boolean AFEAPIHTTPDomoticz::publishGateState(uint8_t id) {
  return _initialized ? sendSwitchCommand(_Hardware->Gate[id]->configuration->domoticz.idx,
...
boolean AFEAPIHTTPDomoticz::publishContactronState(uint8_t id) {
  return _initialized
             ? sendSwitchCommand(_Hardware->Contactron[id]->configuration->domoticz.idx,
...
boolean AFEAPIHTTPDomoticz::publishDS18B20SensorData(uint8_t id) {
  ...
  if (_initialized) {
    if (_Hardware->DS18B20Sensor[id]->configuration->domoticz.idx > 0) {
      ...
      sendCustomSensorCommand(_Hardware->DS18B20Sensor[id]->configuration->domoticz.idx,
                              value);
      _ret = true;
    }
  }
...
boolean AFEAPIHTTPDomoticz::publishRegulatorState(uint8_t id) {
  return _initialized
             ? sendSwitchCommand(_Hardware->Regulator[id]->configuration->domoticz.idx,
...
boolean AFEAPIHTTPDomoticz::publishDHTSensorData(uint8_t id) {
  ...
  if (_initialized) {
    ...
    if (_Hardware->DHTSensor[id]->configuration->domoticz.temperature.idx > 0) {
      ...
      sendCustomSensorCommand(
          _Hardware->DHTSensor[id]->configuration->domoticz.temperature.idx, value);
      _ret = true;
    }

    /* Humidity */
    if (_Hardware->DHTSensor[id]->configuration->domoticz.humidity.idx > 0) {
      sprintf(value, "%d", _Hardware->DHTSensor[id]->convertHumidyStatusDomoticz(
    ...
    /* Absolute Humidity */
    if (_Hardware->DHTSensor[id]->configuration->domoticz.absoluteHumidity.idx > 0) {
      sprintf(value, "%d", _Hardware->DHTSensor[id]->convertHumidyStatusDomoticz(
                               _Hardware->DHTSensor[id]->currentHumidity));
      sendCustomSensorCommand(
          _Hardware->DHTSensor[id]->configuration->domoticz.absoluteHumidity.idx, value,
      ...
      sprintf(value, "%-.2f",
              _Hardware->DHTSensor[id]->dewPoint(
                  ...
                  _Hardware->DHTSensor[id]->configuration->temperature.unit ==
                      AFE_TEMPERATURE_UNIT_FAHRENHEIT));
      sendCustomSensorCommand(
          _Hardware->DHTSensor[id]->configuration->domoticz.dewPoint.idx, value);
      _ret = true;
    }
      ...
      sprintf(value, "%-.2f",
              _Hardware->DHTSensor[id]->heatIndex(
                  ...
                  _Hardware->DHTSensor[id]->configuration->temperature.unit ==
                      AFE_TEMPERATURE_UNIT_FAHRENHEIT));
      sendCustomSensorCommand(
          _Hardware->DHTSensor[id]->configuration->domoticz.heatIndex.idx, value);
      _ret = true;
    }

    /* Temperature & Humidity */
    if (_Hardware->DHTSensor[id]->configuration->domoticz.temperatureHumidity.idx > 0) {
      sprintf(value, "%-.1f;%-.1f;%-d", _Hardware->DHTSensor[id]->currentTemperature,
              _Hardware->DHTSensor[id]->currentHumidity,
              _Hardware->DHTSensor[id]->convertHumidyStatusDomoticz(
                  _Hardware->DHTSensor[id]->currentHumidity));
      sendCustomSensorCommand(
          _Hardware->DHTSensor[id]->configuration->domoticz.temperatureHumidity.idx,
          value);
      _ret = true;
    }
    ...
    if (_Hardware->DHTSensor[id]->configuration->domoticz.comfort.idx > 0) {
      ...
      char _charText[100]; // Max size of Comfort,Perception from lang.pack +
                           // %20 instead of space
      String _stringText;
      _Hardware->DHTSensor[id]->comfort(comfortStatus, _Hardware->DHTSensor[id]->currentTemperature,
                              _Hardware->DHTSensor[id]->currentHumidity,
                              _Hardware->DHTSensor[id]->configuration->temperature.unit ==
                                  AFE_TEMPERATURE_UNIT_FAHRENHEIT);
      ...
      sendCustomSensorCommand(
          _Hardware->DHTSensor[id]->configuration->domoticz.comfort.idx, _charText,
    ...
    if (_Hardware->DHTSensor[id]->configuration->domoticz.perception.idx > 0) {
      ...
      byte _perceptionId = _Hardware->DHTSensor[id]->perception(
          _Hardware->DHTSensor[id]->currentTemperature, _Hardware->DHTSensor[id]->currentHumidity,
          _Hardware->DHTSensor[id]->configuration->temperature.unit ==
              AFE_TEMPERATURE_UNIT_FAHRENHEIT);
      strcpy_P(_charText,
               (char *)pgm_read_dword(&(dewPointPerception[_perceptionId])));
      ...
      sendCustomSensorCommand(
          _Hardware->DHTSensor[id]->configuration->domoticz.perception.idx, _charText,
...
boolean AFEAPIHTTPDomoticz::publishBinarySensorState(uint8_t id) {
  return _initialized
             ? sendSwitchCommand(_Hardware->BinarySensor[id]->configuration->domoticz.idx,
  ...
  if (_initialized) {
    if (_Hardware->MiFareCard[id]->configuration.domoticz[tagId].idx > 0) {
      publishStatus = sendCustomSensorCommand(
          _Hardware->MiFareCard[id]->configuration.domoticz[tagId].idx, user, state);
    }
  }
