...
#ifdef DEBUG
  _Firmware->Debugger->printInformation(F("Sending current device state to MQTT Broker"), F("MQTT"));
#endif
...
#ifdef DEBUG
  _Firmware->Debugger->printTextHeader(F("Subscribing to MQTT Topics"), 50, AFE_DEBUG_HEADER_TYPE_DASH);
#endif
...
void AFEAPIMQTTStandard::listener()
{
  ...
  {
#ifdef DEBUG
    _Firmware->Debugger->printTextHeader(F("MQTT message recieved"), 40, AFE_DEBUG_HEADER_TYPE_DASH);
...
#ifdef DEBUG
    _Firmware->Debugger->printBulletPoint(F("Warning: Topic is NOT defined in this device"));
...
{
#ifdef DEBUG
  _Firmware->Debugger->printProcessingRequest(F("BatteryMeter"), *id,F("MQTT"));
#endif
  if (strcmp(Mqtt->message.content, AFE_CONFIG_MQTT_COMMAND_GET) == 0)
  {
...
{
#ifdef DEBUG
  _Firmware->Debugger->printProcessingRequest(F("BatteryMeter"),AFE_NONE,F("MQTT"));
#endif
  if (strcmp(Mqtt->message.content, AFE_CONFIG_MQTT_COMMAND_GET) == 0)
  {
...
#ifdef DEBUG
  _Firmware->Debugger->printProcessingRequest(F("Themrnal Protector"), *id,F("MQTT"));
#endif
  if (strcmp(Mqtt->message.content, AFE_CONFIG_MQTT_COMMAND_ON) == 0)
  {
...
{
#ifdef DEBUG
  _Firmware->Debugger->printProcessingRequest(F("Binary Sensor"), *id,F("MQTT"));
#endif
  if (strcmp(Mqtt->message.content, AFE_CONFIG_MQTT_COMMAND_GET) == 0)
  {
...
#ifdef DEBUG
  _Firmware->API->Flash->printBufforSizeInfo(AFE_CONFIG_HARDWARE_CLED_CMD_JSON_LENGTH, jsonBuffer.size());
#endif
  if (!root.success())
  {
#ifdef DEBUG
...
#ifdef DEBUG
      _Firmware->Debugger->printInformation(F("Changing color: RGB["), F("RGB LED"));
      Serial << _color.color.red
  ...
  if (enabled && (state == AFE_ON || state == AFE_OFF))
  {
    if (strlen(topic) > 0)
    {
      ...
      publishStatus = Mqtt->publish(
          ...
          : sendAsOpenClosed ? AFE_OPEN_LABEL
                             : AFE_OFF_LABEL);
    }
  }
  return publishStatus;
...
{
#ifdef DEBUG
  _Firmware->Debugger->printInformation(F("Processing: ON/OFF Command"), F("MQTT"));
#endif
  uint8_t _return = AFE_NONE;
  if ((char)command[1] == 'n' && *length == 2)
  { /* ON */
    _return = AFE_ON;
  }
  else if ((char)command[1] == 'f' && *length == 3)
  { /* OFF */
    _return = AFE_OFF;
#ifdef DEBUG
  }
