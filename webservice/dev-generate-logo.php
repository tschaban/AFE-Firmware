...
{
    global $db;
    $q = "INSERT INTO " . C_TABLE_PREFIX . "logs (device_id, user_ip,AFE_FIRMWARE_TYPE, AFE_FIRMWARE_VERSION,firmware_installed_on, last_seen_on, geo_country_code,geo_country_name,geo_region_code,geo_region_name,geo_zipcode,geo_latitude,geo_longitude,geo_city,geo_areacode,geo_metro_code,geo_time_zone )  VALUES ('" . $d . "','" . C_USER_IP . "','" . $t . "','" . $v . "',CURRENT_TIMESTAMP(),CURRENT_TIMESTAMP(),'" . $l->country_code . "','" . $l->country_name . "','" . $l->region_code . "','" . $l->region_name . "','" . $l->zipcode . "','" . $l->latitude . "','" . $l->longitude . "','" . $l->city . "','" . $l->areacode . "','" . $l->metro_code . "','" . $l->time_zone . "')";
    $db->executeQuery($q);
...
{
    global $db;
    $q = "UPDATE " . C_TABLE_PREFIX . "logs SET AFE_FIRMWARE_TYPE = '" . $t . "', AFE_FIRMWARE_VERSION = '" . $v . "', firmware_upgraded_on = CURRENT_TIMESTAMP(), user_ip = '" . C_USER_IP . "', last_seen_on = CURRENT_TIMESTAMP(), geo_country_code='".$l->country_code."',geo_country_name='".$l->country_name."',geo_region_code='".$l->region_code."',geo_region_name='".$l->region_name."',geo_zipcode='".$l->zipcode."',geo_latitude='".$l->latitude."',geo_longitude='".$l->longitude."',geo_city='".$l->city."',geo_areacode='".$l->areacode."',geo_metro_code='".$l->metro_code."',geo_time_zone='".$l->time_zone."' WHERE device_id = '" . $d . "'";
    $db->executeQuery($q);
}
...
{
    global $db;
    $q = "UPDATE " . C_TABLE_PREFIX . "logs SET user_ip = '" . C_USER_IP . "', last_seen_on = CURRENT_TIMESTAMP(),geo_country_code='".$l->country_code."',geo_country_name='".$l->country_name."',geo_region_code='".$l->region_code."',geo_region_name='".$l->region_name."',geo_zipcode='".$l->zipcode."',geo_latitude='".$l->latitude."',geo_longitude='".$l->longitude."',geo_city='".$l->city."',geo_areacode='".$l->areacode."',geo_metro_code='".$l->metro_code."',geo_time_zone='".$l->time_zone."' WHERE user_ip = '" . C_USER_IP . "' AND AFE_FIRMWARE_VERSION = '" . $f . "' AND AFE_FIRMWARE_TYPE='" . $t . "' AND device_id = 'Null'";
    $db->executeQuery($q);
}
...
if ($_device_id != $_empty) {
    ...
    if ($data = $db->fetchRecord($_res)) {
        if ($data["AFE_FIRMWARE_VERSION"] <> $_version || $data["AFE_FIRMWARE_TYPE"] <> $_type) {
            ...
            $notification = new pushover();
            $notification->send("AFE Firmware", "AFE Firmware upgraded\nPrevious version: T" . $data["AFE_FIRMWARE_TYPE"] . "/" . $data["AFE_FIRMWARE_VERSION"] .
